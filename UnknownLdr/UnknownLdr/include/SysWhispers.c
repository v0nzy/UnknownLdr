...
EXTERN_C PVOID SW3_GetRandomSyscallAddress(DWORD FunctionHash)
{
    ...
    // Replace rand() with GetPseudoRandomInteger()
    DWORD index = GetPseudoRandomInteger() % SW3_SyscallList.Count;

    while (FunctionHash == SW3_SyscallList.Entries[index].Hash) {
        SetPseudoRandomSeed(GetTickCount());  // Update the seed
        index = GetPseudoRandomInteger() % SW3_SyscallList.Count;
    }
